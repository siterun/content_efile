...
{	
	public function phptext($text,$textColor,$backgroundColor='',$fontSize,$imgWidth,$imgHeight,$dir,$fileName)
	{
		...
		//if($backgroundColor==''){/*select random color*/                     COMMENT by self	 
			//$colorCode=array('#56aad8', '#61c4a8', '#d3ab92');              COMMENT by self	
			//$backgroundColor = $this->hexToRGB($colorCode[rand(0, count($colorCode)-1)]);COMMENT by self	
			$backgroundColor = imagecolorallocate($im, $backgroundColor['r'],$backgroundColor['g'],$backgroundColor['b']);
		//}else{/*select background color as provided*/                    COMMENT by self	
		//	$backgroundColor = $this->hexToRGB($backgroundColor);           COMMENT by self	
			$backgroundColor = imagecolorallocate($im, $backgroundColor['r'],$backgroundColor['g'],$backgroundColor['b']);
		//}
		
		imagefill($im,0,0,$backgroundColor);	
		//list($x, $y) = $this->ImageTTFCenter($im, $text, $font, $fontSize);  COMMENT by self		
	...
	{	
		...
		//if($noiceLines>0){        COMMENT by self	
